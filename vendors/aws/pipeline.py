...
class DuckDBS3Reader:
    """Unified DuckDB S3 file reader for CSV and Parquet files."""
    ...
    def __enter__(self):
        """Context manager entry - setup DuckDB connection."""
        ...
        self.conn.execute(f"SET s3_access_key_id='{self.aws_creds['access_key_id']}'")
        self.conn.execute(f"SET s3_secret_access_key='{self.aws_creds['secret_access_key']}'")
        self.conn.execute(f"SET s3_region='{self.aws_creds.get('region', 'us-east-1')}'")
    ...
    def read_file(self, bucket: str, key: str, file_format: str) -> Iterator[Dict[str, Any]]:
        """Read file from S3 and yield records as dictionaries."""
        s3_path = f"s3://{bucket}/{key}"
        
        # Read file based on format
        if file_format == 'csv':
            if key.endswith('.gz'):
                result = self.conn.execute(f"SELECT * FROM read_csv_auto('{s3_path}', compression='gzip')").fetchall()
            else:
                result = self.conn.execute(f"SELECT * FROM read_csv_auto('{s3_path}')").fetchall()
        elif file_format == 'parquet':
            result = self.conn.execute(f"SELECT * FROM read_parquet('{s3_path}')").fetchall()
        else:
            raise ValueError(f"Unsupported file format: {file_format}")
        
        columns = [desc[0] for desc in self.conn.description]
        
        print(f"    Loaded {len(result)} rows from {file_format} file")
        print(f"    Columns: {len(columns)}")
        
        # Yield records as dictionaries with cleaned column names
        for row in result:
            record = dict(zip(columns, row))
            yield self._clean_record(record)
    
    def _clean_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        """Clean column names by replacing '/' with '_'."""
        cleaned_record = {}
        for col, val in record.items():
            # Replace / with _ (e.g., "lineItem/UnblendedCost" -> "lineItem_UnblendedCost")
            clean_col = col.replace('/', '_') if '/' in col else col
...
def billing_period_resource(
    ...
    for report_key in manifest.report_keys:
        ...
        # Yield records from the file using unified reader
        if data_reader and hasattr(data_reader, 'read_parquet_file'):
            # Use backend-specific reader if available
            ...
            # Use unified DuckDB reader
            with DuckDBS3Reader(aws_creds) as reader:
                yield from reader.read_file(s3_bucket, s3_key, file_format)
...
def _run_single_table_strategy(config: AWSConfig, backend, state_manager, data_reader, pipeline) -> None:
    """Run pipeline using single table strategy with partition replacement."""
...
def _run_separate_tables_strategy(config: AWSConfig, backend, state_manager, data_reader, pipeline) -> None:
    """Run pipeline using separate tables strategy (recommended)."""
...
def run_aws_pipeline(config: AWSConfig, 
    ...
    if table_strategy == "single":
        _run_single_table_strategy(config, backend, state_manager, data_reader, pipeline)
    else:
        _run_separate_tables_strategy(config, backend, state_manager, data_reader, pipeline)
